...
def create_encoder_single_conv(in_chs, out_chs, kernel):
    assert kernel % 2 == 1
    return nn.Sequential(nn.Conv3d(in_chs, out_chs, kernel_size=kernel, padding=(kernel-1)//2),
...
class EncoderInceptionModuleSignle(nn.Module):
    def __init__(self, channels):
        ...
        self.pool3 = nn.MaxPool3d(3, stride=1, padding=1)
        self.pool5 = nn.MaxPool3d(5, stride=1, padding=2)
...
class Encoder(nn.Module):
    ...
    def _create_downsampling_module(self, input_channels, pooling_kenel):
        return nn.Sequential(
            nn.AvgPool3d(pooling_kenel),
            nn.Conv3d(input_channels, input_channels*2, kernel_size=1),
    ...
    def forward(self, x):
        ...
        out = F.avg_pool3d(out, 8) # Global Average pooling
        return out.view(-1, 256)

## Decoder
def create_decoder_single_conv(in_chs, out_chs, kernel):
    assert kernel % 2 == 1
    return nn.Sequential(nn.ConvTranspose3d(in_chs, out_chs, kernel_size=kernel, padding=(kernel-1)//2),
                            nn.BatchNorm3d(out_chs),
                            nn.ReLU(inplace=True))
...
class Decoder(nn.Module):
    def __init__(self, use_inception, repeat_per_module):
        ...
        self.last = nn.ConvTranspose3d(32, 1, kernel_size=1)

    def _create_upsampling_module(self, input_channels, pooling_kenel):
        return nn.Sequential(
            nn.ConvTranspose3d(input_channels, input_channels//2, kernel_size=pooling_kenel, stride=pooling_kenel),
            nn.BatchNorm3d(input_channels//2),
            nn.ReLU(inplace=True)
        )
